...
for epoch in range(100):
    ...
    for i, data in (pbar := tqdm(enumerate(train_loader), total=train_loader.__len__())):
        ...
        nll = info['nll'].mean()
        mae_dx = info['mae_dx'].mean()
        mae_loc = info['mae_loc'].mean()

        nllloss = nll.mean()

        loss = alpha * nllloss + (1-alpha) * (mae_dx + mae_loc)
        # loss = nll.mean()
        ...
        pbar.set_description(
            f"Epoch {epoch} | Loss {loss.item():.4f} | NLL {nllloss.item():.4f} | MAE_dx {mae_dx.item():.4f} | MAE_loc {mae_loc.item():.4f}")

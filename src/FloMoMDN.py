...
class FloMo(nn.Module):
    ...
    def __init__(self, hist_size, pred_steps, alpha, beta, gamma, num_in=2, B=15., momentum=0.2,
                 ...
                 encoding_type="absdev", hidden=64, n_components=100):
        super().__init__()
        self.pred_steps = pred_steps
        self.n_components = n_components
        self.output_size = pred_steps * num_in * 2 * n_components
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
        self.B = B
        ...
        self.mlp_w = nn.Sequential(
            ...
            nn.Linear(self.obs_encoding_size, self.obs_encoding_size),
            nn.ReLU(),
            nn.Linear(self.obs_encoding_size, self.n_components),
        )
    ...
    def log_prob(self, y_true, x):
        mu, sigma, log_w = self.predict(x)

        y_true = y_true.reshape(-1, self.pred_steps*self.num_in)
        ...
        # # compute log prob for mixture of gaussians
        # log_probs = []
        # for i in range(self.n_components):
        #     normal = Normal(mu[:, i], sigma[:, i], validate_args=True)
        #     log_probs.append(normal.log_prob(y_true).sum(-1))

        # log_probs = torch.stack(log_probs, dim=1)
        # log_probs = torch.logsumexp(log_probs + log_w, dim=1)
        log_probs = mix_dist.log_prob(y_true)
        sigma_reg = sigma.abs().mean()
        return log_probs, sigma_reg
    ...
    def predict(self, x):
        ...
        y_pred = self.mlp(x_enc)
        y_pred = y_pred.view(-1, self.n_components, self.pred_steps, self.num_in, 2)
        ...
        w = self.mlp_w(x_enc)
        log_w = torch.log_softmax(w, dim=-1)

        mu = mu.view(-1, self.n_components, self.pred_steps * self.num_in)
        sigma = sigma.view(-1, self.n_components, self.pred_steps * self.num_in)

        return mu, sigma, log_w

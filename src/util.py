...
def feat_normalization(feature, feature_boundary):
    # feature : [B * T * F]
    # feature boundary : list of lists containing [min, max] of each feature

    # min-max normalization of feature
    feature = (feature - feature_boundary[:, 0]) / (feature_boundary[:, 1] - feature_boundary[:, 0])
    return feature
...
class sequence_data(Dataset):
    def __init__(self, input, feature):
        ...
        assert self.input.shape[0] == self.feature.shape[0]
...
# check if there is any differences between matching
def check_difference(data_input, data_filelist, data_len, sequencelist):
    file_l = []
    for i in range(len(data_input)):
        seq = sequencelist[i]
        file = data_filelist[i]
        if data_len[i] == sum(np.array(seq) < 230):
            pass
        else:
            file_l.append(file)
            print(i)
